...
FR-032～FR-041 已完成项目文档处理、字段草稿、人工确认、清单关联、正式目录、单轮检索问答、物理删除和脱敏审计接入。FR-042 新增独立“档案助手”入口：可在当前项目创建会话；P08 已实现刷新后自动恢复当前项目最近一个会话、加载完整可见历史和脱敏工具调用记录。它不替换 FR-039 单轮诊断入口，也不提供通用会话列表、任意旧会话选择、重命名或删除。
...
- FR-042：独立档案助手 DTO、五端点 API、Store、页面与自动化测试已完成；P08 会在进入页面及登录状态恢复后读取当前项目最近会话；当前路由为 `/projects/{project_id}/archive-agent`。
...
- FR-031～FR-042 的当前公开契约已接入。FR-042 P08 只自动恢复当前项目最近一个会话，仍不提供通用会话列表或任意旧会话选择，历史引用仍为空且不完整轮次仍隐藏；
- P08 已通过隔离 PostgreSQL + SQLite Checkpoint + Vite + 浏览器整页刷新联动验收；该验收使用固定本地模型写入受控历史，没有调用 DeepSeek；
