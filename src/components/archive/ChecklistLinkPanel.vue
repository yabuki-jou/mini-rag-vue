    ...
    <section class="dashboard-card checklist-link-panel" data-testid="checklist-link-panel">
        ...
        <div v-else class="checklist-link-list">
            <article v-for="item in checklistItems" :key="item.id" class="checklist-link-row">
                ...
                <div v-if="linkFor(item.id)?.status === 'INVALIDATED'" class="invalidated-detail">失效原因：{{ linkFor(item.id)?.invalidated_reason || '服务端未提供原因。' }}</div>
                <div class="row-actions">
                    <button v-if="linkFor(item.id)?.status !== 'CONFIRMED'" class="primary-button" :data-testid="`create-checklist-link-${item.id}`" :disabled="!confirmed || loading[`create-checklist-link:${document.id}:${item.id}`]" @click="requestCreate(item)">
                        {{ linkFor(item.id)?.status === 'INVALIDATED' ? '重新确认关联' : '确认关联' }}
                    </button>
                    <button v-if="linkFor(item.id)" class="danger-button sm" :data-testid="`delete-checklist-link-${linkFor(item.id)!.id}`" :disabled="loading[`delete-checklist-link:${linkFor(item.id)!.id}`]" @click="requestDelete(linkFor(item.id)!)">删除关联</button>
                </div>
            </article>
        </div>
        ...
        <div v-if="links.length && links.some(link => !checklistItems.some(item => item.id === link.checklist_item_id))" class="invalidated-links">
            ...
            <p v-for="link in links.filter(entry => !checklistItems.some(item => item.id === entry.checklist_item_id))" :key="link.id">
                ...
                <button class="danger-button sm" :data-testid="`delete-checklist-link-${link.id}`" @click="requestDelete(link)">删除关联</button>
            </p>
        </div>
    </section>
</template>
