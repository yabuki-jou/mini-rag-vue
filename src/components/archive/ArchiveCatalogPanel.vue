...
const documentTypes: Array<[ArchiveDocumentType, string]> = [
    ['CONTRACT', '合同'],
    ['DESIGN', '设计资料'],
    ['CONSTRUCTION', '施工资料'],
...
const projectStages: Array<[ProjectStage, string]> = [
    ['PREPARATION', '准备阶段'],
    ['DESIGN', '设计阶段'],
    ['CONSTRUCTION', '施工阶段'],
    ['ACCEPTANCE', '验收阶段'],
...
watch(
    ...
    filters => {
        Object.assign(form, {
            ...
            project_stage: filters.project_stage || '',
            document_date_from: filters.document_date_from || '',
            document_date_to: filters.document_date_to || '',
            document_date_is_null: filters.document_date_is_null || false,
            authoring_organization: filters.authoring_organization || ''
        })
    },
...
<template>
    <section class="archive-catalog-panel" data-testid="archive-catalog-panel">
        <header class="catalog-header">
            <div>
                <span class="eyebrow">FR-038 · FORMAL ARCHIVE CATALOG</span>
                <h1>正式档案目录</h1>
                <p>只展示服务端已确认并归属于当前项目的档案。</p>
            </div>
        </header>
        <form class="catalog-filters" @submit.prevent="applyFilters">
            <select data-testid="archive-document-type-filter" v-model="form.document_type">
                <option value="">全部资料类型</option>
                <option v-for="[value, label] in documentTypes" :key="value" :value="value">{{ label }}</option>
            </select>
            <select v-model="form.project_stage">
                <option value="">全部项目阶段</option>
                <option v-for="[value, label] in projectStages" :key="value" :value="value">{{ label }}</option>
            </select>
            <input data-testid="archive-date-from" v-model="form.document_date_from" type="date" aria-label="起始日期" :disabled="form.document_date_is_null" /><input
                data-testid="archive-date-to"
                v-model="form.document_date_to"
                type="date"
                aria-label="结束日期"
                :disabled="form.document_date_is_null" />
        ...
        <div v-if="loading.archives && !archives.length" class="archive-loading" data-testid="archive-loading"><span class="spinner" aria-hidden="true"></span>正在加载正式档案…</div>
        <div v-else-if="!archives.length" class="archive-empty">当前筛选下没有正式档案。</div>
        <section v-else class="archive-table">
            ...
            <article v-for="archive in archives" :key="archive.id" class="archive-row">
                ...
                ><span>{{ archive.title || '（无标题）' }}</span
                ><span>{{ archive.document_type ? typeLabels[archive.document_type] : '—' }}</span
                ><span>{{ archive.project_stage ? stageLabels[archive.project_stage] : '—' }}</span
                ><span>{{ archive.authoring_organization || '—' }}</span
                ><span>{{ archive.document_date || '—' }}</span
                ><span>{{ archive.confirmed_at }}</span
                ><button class="link-button" @click="emit('open-detail', archive.id)">查看详情</button>
            </article>
        </section>
        <footer class="catalog-pagination">
            <span>共 {{ total }} 份 · 第 {{ page }} 页</span><button data-testid="archive-prev-page" :disabled="!canPrev || loading.archives" @click="emit('page-change', page - 1)">{{ loading.archives ? '加载中…' : '上一页' }}</button
            ><button data-testid="archive-next-page" :disabled="!canNext || loading.archives" @click="emit('page-change', page + 1)">{{ loading.archives ? '加载中…' : '下一页' }}</button>
        </footer>
        <aside v-if="currentArchive" class="archive-detail" data-testid="archive-detail">
            <div class="card-heading">
                <div>
                    <span class="eyebrow">ARCHIVE DETAIL</span>
                    <h2>{{ currentArchive.title || currentArchive.filename }}</h2>
                </div>
                <span>v{{ currentArchive.version }}</span>
            </div>
            <p>{{ archiveDateLabel(currentArchive.document_type) }}：{{ currentArchive.document_date || '（未提供）' }}</p>
            <article v-for="field in currentArchive.fields" :key="field.id" class="archive-field">
                <strong>{{ fieldLabel(field.field_name, currentArchive.document_type) }}</strong
                ><span>{{ fieldValue(field) }}</span
                ><small v-for="evidence in field.evidences" :key="evidence.id">{{ evidenceLocation(evidence) }}：{{ evidence.excerpt }}</small>
            </article>
        </aside>
    </section>
</template>
...
.archive-row {
    display: grid;
    grid-template-columns: 1.4fr 1.4fr 0.9fr 0.9fr 1.1fr 0.9fr 1.2fr 0.7fr;
    gap: 12px;
    align-items: center;
    padding: 13px 16px;
}
