...
<template>
    <section class="document-panel">
        ...
        <section class="document-metrics" aria-label="文档处理统计">
            <article>
                <span>文档总数</span><strong data-testid="document-total">{{ total }}</strong>
            </article>
            <article>
                <span>待解析</span><strong>{{ parsingCount }}</strong>
            </article>
            <article>
                <span>解析失败</span><strong>{{ failedCount }}</strong>
            </article>
        </section>

        <div class="document-toolbar">
            <label
                >状态筛选<select data-testid="document-status-filter" :value="status || ''" @change="emit('status-change', ($event.target as HTMLSelectElement).value as ProcessDocument['status'] | '')">
                    <option value="">全部状态</option>
                    <option v-for="(label, value) in statusLabels" :key="value" :value="value">{{ label }}</option>
                </select></label
            >
            <span>共 {{ total }} 份 · 第 {{ currentPage }} 页</span>
            <button data-testid="document-prev-page" :disabled="!canPrev || loading['project-documents']" @click="emit('page-change', currentPage - 1)">{{ loading['project-documents'] ? '加载中…' : '上一页' }}</button>
            <button data-testid="document-next-page" :disabled="!canNext || loading['project-documents']" @click="emit('page-change', currentPage + 1)">{{ loading['project-documents'] ? '加载中…' : '下一页' }}</button>
        </div>

        <div v-if="loading['project-documents'] && documents.length === 0" class="document-loading" data-testid="document-loading"><span class="spinner" aria-hidden="true"></span>正在加载文档列表…</div>
        <div v-else-if="documents.length === 0" class="document-empty">当前项目还没有上传资料。</div>
        <section v-else class="document-table" aria-label="项目文档处理列表">
            ...
            <article v-for="document in documents" :key="document.id" class="document-row">
                <div>
                    ...
                    ><small>{{ document.file_hash.slice(0, 12) }}…</small>
                </div>
                <div>
                ...
                <div>
                    <b>{{ document.field_summary.checked_count }}/{{ document.field_summary.total_count }}</b
                    ><small>已检查字段</small>
                </div>
                <div>
...
.document-row {
    display: grid;
    grid-template-columns: 2fr 1.25fr 1fr 0.8fr 1.1fr;
    gap: 14px;
    align-items: center;
    padding: 14px 18px;
}
...
.status-pill.suggestion_failed,
.status-pill.pending_confirmation,
.status-pill.pending_reconfirmation {
    background: #fff7e6;
    color: #b45309;
}
