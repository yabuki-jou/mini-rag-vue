...
const documentTypes: Array<{ value: ArchiveDocumentType; label: string }> = [
  { value: 'CONTRACT', label: '合同' }, { value: 'DESIGN', label: '设计' }, { value: 'CONSTRUCTION', label: '施工' },
  { value: 'MEETING_MINUTES', label: '会议纪要' }, { value: 'ACCEPTANCE', label: '验收' }, { value: 'OTHER', label: '其他' },
]
const projectStages: Array<{ value: ProjectStage; label: string }> = [
  { value: 'PREPARATION', label: '准备' }, { value: 'DESIGN', label: '设计' }, { value: 'CONSTRUCTION', label: '施工' },
  { value: 'ACCEPTANCE', label: '验收' }, { value: 'CROSS_STAGE', label: '跨阶段' }, { value: 'OTHER_STAGE', label: '其他阶段' },
]
...
function canRegenerate() {
  return props.draft.document.status === 'PENDING_CONFIRMATION'
    && props.draft.fields.length === fieldOrder.length
    && props.draft.fields.every((field) => field.review_status === 'PENDING_CHECK' && field.source !== 'MANUAL' && !field.no_source_evidence)
}
...
function addEvidence(fieldName: ArchiveFieldName) {
  ...
  localEvidences[fieldName] = [...(localEvidences[fieldName] || []), {
    excerpt: '', location_type: 'TEXT_LINE_RANGE', location_start: 1, location_end: 1, normalized_anchor: null,
  }]
}
...
function statusesFor(fieldName: ArchiveFieldName): FieldReviewStatus[] {
  return fieldName === 'TITLE' || fieldName === 'DOCUMENT_TYPE'
    ? ['PENDING_CHECK', 'VALUE_CONFIRMED']
    : ['PENDING_CHECK', 'VALUE_CONFIRMED', 'EMPTY_ACCEPTED']
}
...
function saveField(field: ArchiveFieldDraft) {
  ...
  if (reviewStatus !== 'EMPTY_ACCEPTED') {
    ...
    else if (field.field_name === 'KEYWORDS') payload.json_value = value.split(',').map((item) => item.trim()).filter(Boolean)
...
function locationText(field: ArchiveFieldDraft) {
  return field.evidences.map((evidence) => `${evidence.location_type} ${evidence.location_start}–${evidence.location_end}`).join('；')
}
</script>
  ...
  <section class="draft-panel" data-testid="archive-draft-panel">
    <header class="draft-header">
      ...
      <div class="draft-actions">
        <button v-if="draft.document.status === 'PENDING_CONFIRMATION'" data-testid="confirm-archive" class="primary-button" :disabled="!allFieldsChecked() || loading[`confirm-archive-document:${draft.document.id}`]" @click="emit('confirm', draft.document.version)">{{ allFieldsChecked() ? '确认并正式入档' : `确认并正式入档（还剩 ${uncheckedCount()} 个字段待检查）` }}</button>
        <button v-if="draft.document.status === 'PENDING_RECONFIRMATION'" data-testid="reconfirm-archive" class="primary-button" :disabled="!allFieldsChecked() || loading[`confirm-archive-document:${draft.document.id}`]" @click="emit('confirm', draft.document.version)">重新确认并正式入档</button>
    ...
    <div class="draft-fields">
      <article v-for="fieldName in fieldOrder" :key="fieldName" class="draft-field" :data-testid="`draft-field-${fieldName}`">
        <div class="field-heading"><label :for="`field-${fieldName}`">{{ fieldLabels[fieldName] }}</label><span>{{ fieldByName(fieldName)?.review_status || 'PENDING_CHECK' }}</span></div>
        <select :data-testid="`review-status-${fieldName}`" :value="localStatuses[fieldName]" :disabled="!canEdit()" @change="setStatus(fieldName, ($event.target as HTMLSelectElement).value)">
          <option v-for="status in statusesFor(fieldName)" :key="status" :value="status">{{ status }}</option>
        </select>
        <select v-if="fieldName === 'DOCUMENT_TYPE'" :id="`field-${fieldName}`" :data-testid="`field-${fieldName}`" :value="localValues[fieldName]" :disabled="!canEdit()" @change="setValue(fieldName, ($event.target as HTMLSelectElement).value)">
          <option value="">请选择资料类型</option><option v-for="option in documentTypes" :key="option.value" :value="option.value">{{ option.label }}</option>
        </select>
        <select v-else-if="fieldName === 'PROJECT_STAGE'" :id="`field-${fieldName}`" :data-testid="`field-${fieldName}`" :value="localValues[fieldName]" :disabled="!canEdit()" @change="setValue(fieldName, ($event.target as HTMLSelectElement).value)">
          <option value="">请选择项目阶段</option><option v-for="option in projectStages" :key="option.value" :value="option.value">{{ option.label }}</option>
        </select>
        ...
        <label class="no-evidence-toggle" v-if="canEdit()"><input :data-testid="`no-evidence-${fieldName}`" type="checkbox" :checked="localNoEvidence[fieldName]" :disabled="Boolean(localEvidences[fieldName]?.length)" @change="toggleNoEvidence(fieldName, ($event.target as HTMLInputElement).checked)" />确认无原文证据</label>
        <div v-if="canEdit() && !localNoEvidence[fieldName]" class="evidence-editor" :data-testid="`evidence-editor-${fieldName}`">
          <div v-for="(evidence, evidenceIndex) in localEvidences[fieldName]" :key="`${fieldName}-${evidenceIndex}`" class="evidence-row">
            <input :data-testid="`evidence-excerpt-${fieldName}-${evidenceIndex}`" :value="evidence.excerpt" placeholder="证据摘录" @input="setEvidenceValue(fieldName, evidenceIndex, 'excerpt', ($event.target as HTMLInputElement).value)" />
            <select :data-testid="`evidence-location-${fieldName}-${evidenceIndex}`" :value="evidence.location_type" @change="setEvidenceValue(fieldName, evidenceIndex, 'location_type', ($event.target as HTMLSelectElement).value)"><option value="PDF_PAGE">PDF_PAGE</option><option value="DOCX_PARAGRAPH">DOCX_PARAGRAPH</option><option value="TEXT_LINE_RANGE">TEXT_LINE_RANGE</option></select>
            ...
            <input v-if="evidence.location_type === 'TEXT_LINE_RANGE'" :data-testid="`evidence-end-${fieldName}-${evidenceIndex}`" type="number" min="1" :value="evidence.location_end" @input="setEvidenceValue(fieldName, evidenceIndex, 'location_end', ($event.target as HTMLInputElement).value)" />
            <input :data-testid="`evidence-anchor-${fieldName}-${evidenceIndex}`" :value="evidence.normalized_anchor || ''" placeholder="可选定位锚点" @input="setEvidenceValue(fieldName, evidenceIndex, 'normalized_anchor', ($event.target as HTMLInputElement).value)" />
            <button type="button" class="link-button" :data-testid="`evidence-remove-${fieldName}-${evidenceIndex}`" @click="removeEvidence(fieldName, evidenceIndex)">删除证据</button>
          </div>
          <button type="button" class="link-button" :data-testid="`evidence-add-${fieldName}`" @click="addEvidence(fieldName)">添加证据</button>
        </div>
        <div class="field-meta"><span>{{ fieldByName(fieldName)?.source === 'AI' ? 'AI 建议' : fieldByName(fieldName)?.source === 'MANUAL' ? '人工填写' : '尚无来源' }}</span><span v-if="fieldByName(fieldName)?.evidences.length">证据：{{ locationText(fieldByName(fieldName)!) }}</span><span v-else-if="localNoEvidence[fieldName]">人工填写、无原文证据</span><span v-else-if="localValues[fieldName]">非空人工值需要明确勾选无原文证据或提供证据</span></div>
        <small v-if="fieldByName(fieldName)?.evidences.length" class="evidence-excerpt">{{ fieldByName(fieldName)?.evidences.map((evidence) => evidence.excerpt).join('；') }}</small>
...
<style scoped>
.draft-panel{display:grid;gap:20px}.draft-header{display:flex;justify-content:space-between;gap:18px;align-items:start}.draft-header h1{margin:5px 0}.draft-header p{margin:0;color:#64748b}.draft-actions{display:flex;gap:10px;flex-wrap:wrap}.draft-notice{padding:12px 16px;border:1px solid #bfdbfe;border-radius:10px;background:#eff6ff;color:#1e40af}.draft-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.draft-field{display:grid;gap:8px;padding:16px;border:1px solid #e2e8f0;border-radius:12px;background:#fff}.field-heading{display:flex;justify-content:space-between;gap:12px;font-weight:700}.field-heading span,.field-meta,.evidence-excerpt{color:#64748b;font-size:12px}.draft-field input,.draft-field select{border:1px solid #cbd5e1;border-radius:8px;padding:10px 12px;font:inherit}.field-meta{display:flex;gap:10px;flex-wrap:wrap}.evidence-excerpt{line-height:1.5}.save-field{justify-self:start}@media(max-width:900px){.draft-header{flex-direction:column}.draft-fields{grid-template-columns:1fr}}
</style>
