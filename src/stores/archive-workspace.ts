...
export const useArchiveWorkspaceStore = defineStore('archive-workspace', {
  ...
  actions: {
    ...
    async startSession(tokens: AuthTokenPair, profile: Pick<User, 'username' | 'name'>) {
      setApiTokens(tokens)
    ...
    async login(payload: AuthLoginRequest) {
      ...
      await this.startSession(tokens, { username: normalizedUsername, name: normalizedUsername })
      return tokens
    },
    async register(payload: AuthRegisterRequest) {
      ...
      if (!tokens) return undefined
      await this.startSession(tokens, registration)
      return registration
    },
    ...
    async loadProjects() {
      ...
      const candidate = this.projects.some((project) => project.id === this.activeProjectId)
        ? this.activeProjectId
        : this.projects.some((project) => project.id === savedProject)
          ? savedProject
          : this.projects[0]?.id || ''
    ...
    async createChecklistItem(payload: ChecklistItemCreate) {
      ...
      this.projects = this.projects.map((project) => project.id === projectId
        ? { ...project, version: response.project_version }
        : project)
    ...
    async deleteProjectDocument(documentId: string) {
      ...
      try {
        ...
        if (this.activeProjectId === projectId) {
          ...
          await Promise.allSettled([
            ...
            this.loadChecklistItems(projectId),
            this.loadArchives(this.archiveFilters, this.archivePage.page, this.archivePage.page_size),
          ])
      ...
      this.projects = this.projects.map((project) => project.id === projectId
        ? { ...project, active_document_count: Math.max(0, project.active_document_count - 1) }
        : project)
      ...
      await Promise.allSettled([
        this.loadProjectDocuments(projectId, targetPage, pageSize, this.projectDocumentStatus),
        this.loadChecklistItems(projectId),
        this.loadArchives(this.archiveFilters, this.archivePage.page, this.archivePage.page_size),
      ])
      return undefined
    ...
    async uploadProjectDocument(file: File) {
      ...
      this.projects = this.projects.map((project) => project.id === projectId
        ? { ...project, active_document_count: project.active_document_count + 1 }
        : project)
      return document
    },
    ...
    syncDraftResponse(projectId: string, documentId: string, draft: ArchiveDraft | undefined) {
      ...
      this.projectDocuments = this.projectDocuments.some((item) => item.id === draft.document.id)
        ? this.projectDocuments.map((item) => item.id === draft.document.id ? draft.document : item)
        : [draft.document, ...this.projectDocuments]
      return draft
    },
    syncDocumentResponse(projectId: string, documentId: string, document: ProcessDocument | undefined) {
      ...
      this.projectDocuments = this.projectDocuments.some((item) => item.id === document.id)
        ? this.projectDocuments.map((item) => item.id === document.id ? document : item)
        : [document, ...this.projectDocuments]
    ...
    async loadChecklistLinkState(documentId: string) {
      ...
      const [suggestions, links] = await Promise.all([
        this.run(`checklist-link-suggestions:${documentId}`, () => api.listChecklistLinkSuggestions(projectId, documentId)),
        this.run(`checklist-links:${documentId}`, () => api.listChecklistLinks(projectId, documentId)),
      ])
