...
const draft = {
  document: {
    id: 'document-1', filename: '施工方案.txt', file_hash: 'a'.repeat(64), status: 'PARSED',
    last_error: { code: null, message: null }, field_summary: { checked_count: 0, total_count: 7 },
    confirmed_at: null, version: 4, uploaded_at: '2026-08-27T00:00:00Z', updated_at: '2026-08-27T00:00:00Z',
  },
  fields: [{
    id: 'field-1', field_name: 'TITLE', text_value: '施工方案', date_value: null, json_value: null,
    review_status: 'PENDING_CHECK', source: 'AI', no_source_evidence: false, updated_by: 'AI',
    updated_at: '2026-08-27T00:00:00Z', evidences: [],
  }],
  snapshot: null,
  next_actions: ['确认字段'],
}
...
const store = {
  isAuthenticated: true, hasSession: true, username: 'demo', userName: '演示用户',
  ...
  activeProjectId: 'project-1', activeProject: null as unknown,
  checklistLinkSuggestions: [], checklistLinks: [],
  projectDocuments: [draft.document], checklistItems: [], currentDraft: draft as typeof draft | null,
  projectDocumentPage: 1, projectDocumentPageSize: 20, projectDocumentTotal: 1, projectDocumentStatus: '',
  archives: [], archivePage: { page: 1, page_size: 20, total: 0 }, archiveFilters: {}, currentArchive: null,
  auditLogs: [], auditPage: { page: 1, page_size: 20, total: 0 }, auditOperationType: '',
  archiveAnswer: null, archiveRetrieval: null,
  loading: {}, error: '', errorStatus: null,
  selectProject: vi.fn(), restoreSession: vi.fn().mockResolvedValue(undefined),
  loadProjectDocuments: vi.fn().mockResolvedValue(undefined), loadChecklistItems: vi.fn(),
  deleteProjectDocument: vi.fn().mockResolvedValue(undefined),
  setProjectDocumentStatus: vi.fn(), loadProjectDocumentPage: vi.fn(),
  loadArchives: vi.fn(), setArchiveFilters: vi.fn(), loadArchivePage: vi.fn(), loadArchiveDetail: vi.fn(),
  loadAuditLogs: vi.fn(), setAuditOperationType: vi.fn(), loadAuditPage: vi.fn(),
  askArchiveQuestion: vi.fn(), retrieveArchives: vi.fn(),
  loadArchiveDraft: vi.fn().mockResolvedValue(draft), createArchiveSuggestions: vi.fn().mockResolvedValue(draft),
  retryArchiveSuggestions: vi.fn(), regenerateArchiveSuggestions: vi.fn(), createManualArchiveDraft: vi.fn(),
  updateArchiveField: vi.fn(), confirmArchiveDocument: vi.fn(), cancelArchiveDocumentConfirmation: vi.fn(),
  loadChecklistLinkState: vi.fn().mockResolvedValue(undefined), createChecklistLink: vi.fn(), deleteChecklistLink: vi.fn(),
  clearError: vi.fn(), signOut: vi.fn(),
  login: vi.fn().mockResolvedValue(undefined), register: vi.fn().mockResolvedValue(undefined),
}
...
describe('ArchiveWorkspaceView FR-034/035 wiring', () => {
  ...
  it('does not delete when the physical-delete confirmation is cancelled', async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith(expect.stringContaining('施工方案.txt'))
  ...
  it('confirms physical deletion once and delegates to the selected project store', async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith(expect.stringContaining('物理删除'))
  ...
  it('opens an existing seven-field draft only from a confirmation-state document row', async () => {
    ...
    const pendingDraft = {
      ...
      fields: ['TITLE', 'DOCUMENT_TYPE', 'DOCUMENT_DATE', 'AUTHORING_ORGANIZATION', 'VERSION_NUMBER', 'PROJECT_STAGE', 'KEYWORDS'].map((fieldName, index) => ({
        ...draft.fields[0], id: `field-${index}`, field_name: fieldName,
      })),
    }
  ...
  it('wires confirm and cancel actions from the draft panel to the selected project store', async () => {
    ...
    const confirmedDraft = {
      ...
      fields: ['TITLE', 'DOCUMENT_TYPE', 'DOCUMENT_DATE', 'AUTHORING_ORGANIZATION', 'VERSION_NUMBER', 'PROJECT_STAGE', 'KEYWORDS'].map((fieldName, index) => ({
        ...draft.fields[0], id: `field-${index}`, field_name: fieldName, review_status: 'VALUE_CONFIRMED' as const,
      })),
    }
